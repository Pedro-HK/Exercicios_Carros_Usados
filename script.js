(function () {
  ...
  const $mediana = document.querySelector(
    ".mediana_anos_carros_selecionados output"
  );
  ...
  const $carrosSelecionados = document.querySelector(
    ".carros_selecionados output"
  );
  const $valorTotalCarros = document.querySelector(
    ".valor_carros_selecionados output"
  );
  const $desvioPadrao = document.querySelector(
    ".desvio_padrao_km_carros_selecionados output"
  );
  ...
  const $carrosFiltrados = document.querySelector(".carros_filtrados output");
  ...
  function putAno(dados) {
    ...
    $anoMaximo.addEventListener("change", (e) => {
      if ($anoMinimo[0].selected) {
        const filtraAnoMinimo = filtraAno.reduce((item, currentItem) => {
          if ($anoMaximo.value >= currentItem) {
            item.push(currentItem);
          }
          return item;
        }, []);

        $anoMinimo.innerHTML = '<option value="-1">-- Selecione --</option>';

        filtraAnoMinimo.map((el) => {
          $anoMinimo.innerHTML += `<option>${el}</option>`;
        });
      }
    });
    ...
    dados.map((dado) => {
      ...
      if (dado.preco === Math.min(...precos)) {
        $promocao.innerHTML = `
            ...
            <h1>25% OFF</h1>
            <div>
              <h2>${mascaraPreco((dado.preco / 4) * 3)}</h2>
              <h4>${mascaraPreco(dado.preco)}</h4>
        ...
        //insere a contagem regressiva de 3 horas
        ...
        $buttonPromocao.addEventListener("click", () => {
          if ($promocao.classList.contains("showPromocao")) {
            ...
            document.querySelector(".icone_promocao").src =
              "/Imagens/circle-down.svg";
          } else {
            $promocao.classList.add("showPromocao");
            document.querySelector(".icone_promocao").src =
              "/Imagens/circle-up.svg";
          }
        });
      }
    });
  ...
  function filter(dados) {
    ...
    const carrosFiltrados = dados.filter((car) => {
      ...
      const filtroCarrosUsados =
        !$carrosNaoUsados.checked &&
        (car.usado === true || car.usado === false);
      const filtroMarca = getMarca() === car.marca || !getMarca();
      const filtroAno =
        (getAnoMinimo() <= car.ano || !getAnoMinimo()) &&
        (getAnoMaximo() >= car.ano || !getAnoMaximo());
      const filtroCombustivel =
        getCombustivel() === car.combustivel || !getCombustivel();
      const filtroPreco =
        ($precoMinimo.value <= car.preco || $precoMinimo.value === "") &&
        ($precoMaximo.value >= car.preco || $precoMaximo.value === "");
      const filtroKm =
        ($kmMinimo.value <= car.km || $kmMinimo.value === "") &&
        ($kmMaximo.value >= car.km || $kmMaximo.value === "");

      return (
        (filtroCarrosNaoUsados || filtroCarrosUsados) &&
        filtroMarca &&
        filtroAno &&
        filtroCombustivel &&
        filtroPreco &&
    ...
    $btnMediaPrecos.addEventListener("click", () => {
      $mediaPrecos.innerText = mascaraPreco(calcularMedia(precosAtualizados));
    });
    ...
    function anoPopular(anosAtualizados) {
      ...
      const num = Object.keys(popular).find(
        (key) => popular[key] === valorMaximo
      );
  ...
  function mostraListaDeCarros(dados) {
    ...
    function calcularMedianaAno(arrayAnos) {
      ...
      let medianaImpar = arrayAnos[arrayAnos.length / 2 - 0.5];
    ...
    // imprimi na tela a quantidade de carros filtrados
    $carrosFiltrados.innerHTML = dados.length;
    ...
    dados.forEach((dado) => {
      ...
      card_carro.innerHTML = `
        <h2>${mascaraPreco(dado.preco)}</h2>
        <h3>Modelo: ${dado.modelo}</h3>
        <h4>Marca: ${dado.marca}</h4>
      ...
      card_carro.addEventListener("click", (e) => {
        ...
        $valorTotalCarros.innerHTML = mascaraPreco(totalValor);
        ...
        if (kmSelecionados.length == 0) {
          ...
          $desvioPadrao.innerHTML =
            calcularDesvioPadrao(kmSelecionados) + " Km";
        }
    ...
    $valorTotalCarros.innerHTML = mascaraPreco(0);
    $mediaPrecos.innerText = mascaraPreco(0);
    $desvioPadrao.innerHTML = 0 + " Km";
    $anoPopular.innerText = 0;
    $mediana.innerHTML = 0;
    $carrosSelecionados.innerHTML = 0;
  }
  ...
  $marcaModelo.addEventListener("input", (e) => {
    const searchCar = $marcaModelo.value
      .normalize("NFD")
      .replace(/[\u0300-\u036f]/g, "");
    ...
    data.then((dados) => {
      ...
      const filteredCars = dados.filter((car) => {
        return (
          car.modelo.toLowerCase().includes(searchCar) ||
          car.marca.toLowerCase().includes(searchCar)
        );
      });
